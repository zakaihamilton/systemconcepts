const nextJest = require("next/jest") as (options: {
	dir: string;
...
const customJestConfig = {
	...
	moduleNameMapper: {
		...
		"^@icons/Audio$": "<rootDir>/src/components/Icons/Audio.tsx",
	...
	transform: {
		"^.+\\.svg$": [
			"<rootDir>/jest.svgr-transformer.ts",
...
module.exports = async () => {
	...
	if (Array.isArray(jestConfig.moduleNameMapper)) {
		jestConfig.moduleNameMapper = jestConfig.moduleNameMapper.filter(
			([pattern]: [string, string | string[]]) => pattern !== svgMockPattern,
		);
	} else {
