 ...
 */
export async function updateManifestEntry(manifestPath: any, entry: any) {
	let manifest: any[] = [];
	...
	const index = manifest.findIndex((f: any) => f.path === entry.path);
	if (index !== -1) {
 ...
 */
export async function applyManifestUpdates(baseManifest: any, updates: any) {
