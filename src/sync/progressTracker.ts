...
export const MAIN_SYNC_WEIGHT = Object.values(STEP_WEIGHTS).reduce(
	(sum, w) => sum + w,
	0,
);
export const LIBRARY_SYNC_WEIGHT = Object.values(STEP_WEIGHTS).reduce(
	(sum, w) => sum + w,
	0,
);
export const PERSONAL_SYNC_WEIGHT = Object.values(PERSONAL_STEP_WEIGHTS).reduce(
	(sum, w) => sum + w,
	0,
);
export const TOTAL_COMBINED_WEIGHT =
	MAIN_SYNC_WEIGHT + LIBRARY_SYNC_WEIGHT + PERSONAL_SYNC_WEIGHT;
...
export class SyncProgressTracker {
	...
	constructor(
		...
		this.localTotalWeight = Object.values(this.weights).reduce(
			(sum, w) => sum + w,
			0,
		);
