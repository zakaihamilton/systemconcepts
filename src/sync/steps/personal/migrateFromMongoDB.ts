...
type MigrationFile = { path: string; mtimeMs: number };
type MigrationState = {
	files: MigrationFile[];
	migrated: Record<string, boolean>;
	complete: boolean;
};
type ManifestEntry = { path: string; version?: number; [key: string]: any };
type BundleCache = {
...
export async function migrateFromMongoDB(
	...
	const remotePathSet = new Set<string>(
		Array.isArray(remoteManifest) ? remoteManifest.map((f) => f.path) : [],
	);
	addSyncLog(
		`Migration check: Remote manifest has ${remotePathSet.size} files (Array check: ${Array.isArray(remoteManifest)})`,
		"verbose",
	);
	...
	const hasRemoteFiles = Array.from(remotePathSet).some(
		(key) => key.endsWith(".json") && !key.endsWith("migration.json"),
	);
	...
	const safeWriteMigration = async (data: any, _stage?: string) => {
	...
	try {
		...
		if (await storage.exists(migrationPath)) {
			try {
				...
				migrationState = JSON.parse(content) as MigrationState;
				...
				if (migrationState.complete) {
					...
					const remoteHasJson = Array.from(remotePathSet).some((k) =>
						k.endsWith(".json"),
					);
					...
					if (!remoteHasJson && canUpload) {
						...
						const localHasJson = localListing.some(
							(l: any) =>
				...
				addSyncLog(
					`[Personal] Resuming migration: ${Object.keys(migrationState.migrated).length}/${migrationState.files.length} done`,
					"info",
				);
			} catch (err: any) {
				...
				addSyncLog(
					`[Personal] Migration state file corrupted or empty, starting fresh: ${err instanceof Error ? err.message : String(err)}`,
					"info",
				);
		...
		if (migrationState.files.length === 0) {
			...
			if (!(await storage.exists(mongoPath))) {
				...
				await storage.writeFile(
					migrationPath,
					JSON.stringify(migrationState, null, 4),
				);
			...
			// Cache file list
			migrationState.files = files.map((f: any) => ({
				path: f.path,
			...
			addSyncLog(
				`[Personal] Cached ${files.length} files for migration`,
				"info",
			);
		}
		...
		const _getManifestKey = (filePath: any) => {
		...
		let bundledGroups = new Set<any>();
		let mergedGroups = new Set<any>();
		let allGroups = new Set<any>();
		try {
		...
		for (const file of migrationState.files) {
			...
			let relativePath = file.path.substring(
				"personal/metadata/sessions".length + 1,
			);
			relativePath = relativePath.replace(/^[\/\\]+/, "");
		...
		if (hasNewSkippedFiles) {
			addSyncLog(
				`[Personal] Linked ${autoMigratedCount} files to existing remote data`,
				"info",
			);
		...
		const filesToMigrate = migrationState.files.filter(
			(f) => !migrationState.migrated[f.path],
		);
		...
		addSyncLog(
			`[Personal] Migrating ${filesToMigrate.length} remaining files (${done}/${total} done)`,
			"info",
		);

		// Create a map of current manifest entries for updates
		const manifestMap = new Map<string, ManifestEntry>(
			(remoteManifest || []).map((entry: any) => [entry.path, entry]),
		);
		...
		if (await storage.exists(localManifestPath)) {
			try {
				...
				const localManifest = JSON.parse(content) as ManifestEntry[];
		...
		const deletedKeysSet = new Set<any>();

		// Helper to mark key as deleted
		const markAsDeleted = (key: any) => {
		...
		if (migrationState.files && migrationState.files.length > 0) {
			...
			if (repairCount > 0) {
				addSyncLog(
					`[Personal] Found ${repairCount} files with double-slash paths. Queued for repair.`,
					"warning",
				);
		...
		const bundleCache: Record<string, BundleCache> = {};
		...
		for (let i = 0; i < filesToMigrate.length; i += BATCH_SIZE) {
			...
			let batchContents: Record<string, string> | null = null;
			try {
				...
				batch.map(async (file) => {
					try {
						...
						if (
							!relativePath ||
							!relativePath.trim() ||
							relativePath.includes("undefined")
						) {
			...
			await storage.writeFile(
				localManifestPath,
				JSON.stringify(Array.from(manifestMap.values()), null, 4),
			);
			addSyncLog(
				`[Personal] Progress: ${done + migratedCount}/${total} files`,
				"info",
			);
		...
		addSyncLog(
			`[Personal] ✓ Migrated ${migratedCount} files in ${duration}s (${done + migratedCount}/${total} total)`,
			"success",
		);
		...
		return {
			migrated: deletedKeysSet.size > 0 || migratedCount > 0,
		...
		addSyncLog(
			`[Personal] Migration failed: ${err instanceof Error ? err.message : String(err)}`,
			"error",
		);
