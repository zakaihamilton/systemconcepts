import { logger as structuredLogger } from "@util/api/logger";
import { makePath } from "@util/data/path";
import storage from "@util/storage/storage";
import { FILES_MANIFEST, LOCAL_SYNC_PATH, SYNC_BATCH_SIZE } from "../constants";
import { getFileInfo } from "../hash";
import { addSyncLog } from "../logs";
import { lockMutex } from "../mutex";
import { readFileIfExists } from "../storageReads";
import { SyncActiveStore } from "../syncState";

type ManifestEntry = {
	path: string;
	hash?: string;
	size?: number;
	version?: string | number;
	deleted?: boolean;
	[key: string]: any;
};
type ManifestFile = { path: string; fullPath: string };

/**
 * Helper function to compute file info for a single file
 */
async function computeFileInfo(file: ManifestFile) {
	try {
		const unlock = await lockMutex({ id: file.fullPath });
		let content;
		try {
			content = await storage.readFile(file.fullPath);
		} finally {
			unlock();
		}

		if (content === undefined || content === null) {
			return null;
		}
		const info = await getFileInfo(content);
		return { file, info };
	} catch (err: any) {
		structuredLogger.error(`[Sync] Error reading ${file.path}:`, err);
		throw err;
	}
}

/**
 * Step 2: Update local manifest with file hashes
 * Uses parallel batch processing for performance
 */
export async function updateLocalManifest(
	localFiles: ManifestFile[],
	localPath = LOCAL_SYNC_PATH,
	remoteManifest: ManifestEntry[] = [],
	options: Record<string, any> = {},
) {
	const start = performance.now();
	addSyncLog("Step 2: Updating local manifest...", "info");

	const localManifestPath = makePath(localPath, FILES_MANIFEST);
	const remoteManifestMap = new Map<string, ManifestEntry>(
		(remoteManifest || []).map((f) => [f.path, f]),
	);

	try {
		let manifest: ManifestEntry[] = [];
		const existingManifestContent = await readFileIfExists(
			storage,
			localManifestPath,
		);
		if (existingManifestContent) {
			try {
				manifest = JSON.parse(existingManifestContent) as ManifestEntry[];
			} catch (err: any) {
				structuredLogger.error("[Sync] Failed to parse local manifest", err);
				throw new Error(`Invalid local sync manifest at ${localManifestPath}`, {
					cause: err,
				});
			}
		}

		// Create a map for faster lookup
		// Handle old dictionary-style manifest or corruption
		if (manifest && !Array.isArray(manifest)) {
			structuredLogger.debug(
				"[Sync] Converting legacy dictionary manifest to array format",
			);
			manifest = Object.entries(manifest).map(([path, info]) => ({
				path,
				...(info as Record<string, any>),
			}));
		}
		const manifestMap = new Map<string, ManifestEntry>(
			(manifest || []).map((f) => [f.path, f]),
		);

		if (options.skipHashing) {
			addSyncLog(
				`✓ Reused local manifest (${manifest.length} files; hashes skipped)`,
				"info",
			);
			return manifest;
		}

		// Compute file info in parallel batches
		addSyncLog(`Computing hashes for ${localFiles.length} file(s)...`, "info");
		const fileInfos: Array<{ file: ManifestFile; info: any }> = [];

		SyncActiveStore.update((s) => {
			s.progress = { total: localFiles.length, processed: 0 };
		});

		for (let i = 0; i < localFiles.length; i += SYNC_BATCH_SIZE) {
			// Check for cancellation
			if (SyncActiveStore.getRawState().stopping) {
				addSyncLog("Hashing stopped by user", "warning");
				break;
			}
			const batch = localFiles.slice(i, i + SYNC_BATCH_SIZE);
			const progress = Math.min(i + batch.length, localFiles.length);
			const percent = Math.round((progress / localFiles.length) * 100);

			addSyncLog(
				`Computing hashes ${progress}/${localFiles.length} (${percent}%)...`,
				"info",
			);

			SyncActiveStore.update((s) => {
				s.progress = { total: localFiles.length, processed: progress };
			});

			const results = await Promise.all(
				batch.map((file: any) => computeFileInfo(file)),
			);

			fileInfos.push(
				...results.filter(
					(result): result is { file: ManifestFile; info: any } =>
						result !== null,
				),
			);
		}
		if (SyncActiveStore.getRawState().stopping) {
			const error = Object.assign(new Error("Sync stopped during hashing"), {
				code: "SYNC_STOPPED",
			});
			throw error;
		}

		let changed = false;

		// Update manifest with computed info
		for (const { file, info } of fileInfos) {
			if (!manifestMap.has(file.path)) {
				// New file
				const remoteEntry = remoteManifestMap.get(file.path);
				const remoteVer = remoteEntry ? Number(remoteEntry.version) || 0 : 0;
				const newVer = remoteVer + 1;

				const newEntry = {
					path: file.path,
					hash: info.hash,
					size: info.size,
					version: newVer.toString(),
				};
				manifest.push(newEntry);
				manifestMap.set(file.path, newEntry);
				changed = true;
				structuredLogger.debug(
					`[Sync] Added new file to manifest: ${file.path}`,
				);
			} else {
				// Existing file - check if modified
				const existingEntry = manifestMap.get(file.path);
				if (existingEntry && existingEntry.hash !== info.hash) {
					const localVer = Number(existingEntry.version) || 0;
					const remoteEntry = remoteManifestMap.get(file.path);
					const remoteVer = remoteEntry ? Number(remoteEntry.version) || 0 : 0;
					const newVer = Math.max(localVer, remoteVer) + 1;

					existingEntry.hash = info.hash;
					existingEntry.size = info.size;
					existingEntry.version = newVer.toString();
					changed = true;
					structuredLogger.debug(
						`[Sync] Updated file in manifest (version incremented to ${newVer}): ${file.path}`,
					);
				}
			}
		}

		// Mark files as deleted in manifest if they no longer exist locally
		const localFilePaths = new Set(localFiles.map((f: any) => f.path));
		manifest.forEach((f: any) => {
			if (!localFilePaths.has(f.path)) {
				if (!f.deleted) {
					structuredLogger.debug(
						`[Sync] Marking missing file as deleted in manifest: ${f.path}`,
					);
					const localVer = Number(f.version) || 0;
					const remoteEntry = remoteManifestMap.get(f.path);
					const remoteVer = remoteEntry ? Number(remoteEntry.version) || 0 : 0;
					f.version = String(Math.max(localVer, remoteVer) + 1);
					f.deleted = true;
					changed = true;
				}
			} else if (f.deleted) {
				structuredLogger.debug(
					`[Sync] File restored locally, unmarking deleted: ${f.path}`,
				);
				delete f.deleted;
				changed = true;
			}
		});

		if (changed || existingManifestContent === null) {
			const unlock = await lockMutex({ id: localManifestPath });
			try {
				await storage.writeFile(
					localManifestPath,
					JSON.stringify(manifest, null, 4),
				);
			} finally {
				unlock();
			}
		}

		const duration = ((performance.now() - start) / 1000).toFixed(1);
		addSyncLog(
			`✓ Updated manifest in ${duration}s (${manifest.length} files)`,
			"info",
		);
		return manifest;
	} catch (err: any) {
		structuredLogger.error("[Sync] Step 2 error:", err);
		addSyncLog(
			`Step 2 failed: ${err instanceof Error ? err.message : String(err)}`,
			"error",
		);
		throw err;
	}
}
