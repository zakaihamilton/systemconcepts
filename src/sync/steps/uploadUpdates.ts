...
async function uploadFile(
	...
	try {
		...
		structuredLogger.error(`[Sync] Failed to upload ${fileBasename}:`, err);
...
export async function uploadUpdates(
	...
	try {
		const remoteMap = new Map<string, any>(
			(remoteManifest || []).map((f: any) => [f.path, f]),
		);
		const toUpload = [];
		...
		if (progressTracker) {
			progressTracker.updateProgress("uploadUpdates", {
				processed: 0,
		...
		for (let i = 0; i < toUpload.length; i += SYNC_BATCH_SIZE) {
			...
			const results = await Promise.all(
				batch.map((localFile) =>
					uploadFile(localFile, createdFolders, localPath, remotePath),
				),
			);
		...
		addSyncLog(
			`✓ Uploaded ${updates.length} update(s) in ${duration}s`,
