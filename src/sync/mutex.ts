const locks: Record<string, any> = {};

export function getMutex({ id }: any) {
	var lock = locks[id];
	if (!lock) {
		lock = locks[id] = {};
		lock._locking = Promise.resolve();
		lock._locks = 0;
		lock._disabled = false;
		lockMutex({ id }).then((unlock: any) => {
			if (lock._disabled) {
				lock._disabled = unlock;
			} else {
				unlock();
			}
		});
	}
	return lock;
}

export function isMutexLocked({ id }: any) {
	var lock = getMutex({ id });
	if (lock) {
		return lock._locks > 0;
	}
}

export function lockMutex({ id }: any) {
	var lock = getMutex({ id });
	if (lock) {
		lock._locks += 1;
		let unlockNext: any;
		let willLock = new Promise<void>(
			(resolve) =>
				(unlockNext = () => {
					lock._locks -= 1;
					resolve();
				}),
		);
		let willUnlock: any = lock._locking.then(() => unlockNext);
		lock._locking = lock._locking.then(() => willLock);
		return willUnlock;
	}
}
