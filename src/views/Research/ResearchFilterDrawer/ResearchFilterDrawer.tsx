...
export default function ResearchFilterDrawer({
	...
	const filterGroups: Record<string, any[]> = useMemo(() => {
		return availableFilters.reduce((groups: any, filter: any) => {
			(groups[filter.type] ||= []).push(filter);
			return groups;
		}, {});
	}, [availableFilters]);

	const visibleFilterGroups: Record<string, any[]> = useMemo(() => {
		...
		>
			<Box className={styles.drawerContent}>
				...
				<TextField
					...
					onChange={(e: any) => setFilterListQuery(e.target.value)}
					startAdornment={
