...
export default function Research() {
	...
	const [printRoot, setPrintRoot] = useState<any>(null);
	const [filterDrawerOpen, setFilterDrawerOpen] = useState(false);
	const suggestionControlsRef = useRef<any>(null);
	const role = Cookies.get("role");
	const isAdmin = roleAuth(role, "admin");

	const setQuery = useCallback((val: any) => {
		ResearchStore.update((s) => {
			s.query = val;
		});
	}, []);

	const setFilterTags = useCallback((val: any) => {
		ResearchStore.update((s) => {
			s.filterTags = val;
		});
	}, []);

	const setResults = useCallback((val: any) => {
		ResearchStore.update((s) => {
			s.results = val;
		});
	}, []);
	...
	const {
		...
		listRef,
		setResults,
		setSuggestionsOpen: (open: any) =>
			suggestionControlsRef.current?.setSuggestionsOpen(open),
		setActiveSuggestion: (index: any) =>
			...
			{printRoot &&
				createPortal(
					<div className={styles.printContainer}>
						{printing &&
							filteredResults.map((doc: any, index: any) => (
								<div key={doc.docId || index} className={styles.printItem}>
									<Article
										...
										filteredParagraphs={
											doc.matches?.map((m: any) => m.index + 1) || []
										}
										embedded={true}
										hidePlayer={true}
										highlight={highlight}
