...
export function useResearchResultsList({
	...
	const [resultsOutlineAnchor, setResultsOutlineAnchor] = useState<any>(null);
	...
	const pendingPathRef = useRef<any>(null);
	...
	const gotoArticle = useCallback((tag: any, paragraphId: any) => {
		...
		(type: any, value: any) => {
			...
			if (type === "page" && listRef.current) {
				...
				const docIndex = matchCounts.findIndex(
					(m: any) => matchIndex >= m.start && matchIndex <= m.total,
				);
				if (docIndex !== -1) {
	...
	const openResultsOutline = useCallback((event: any) => {
		if (scrollTimeoutRef.current) {
			clearTimeout(scrollTimeoutRef.current);
