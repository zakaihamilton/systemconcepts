	...
	const buildIndex = useCallback(async () => {
		...
		try {
			...
			try {
				...
				if (await storage.exists(groupsPath)) {
					...
					for (const group of groups) {
						const mergedPath = makePath(
							`local/sync/${group.name.replace(/\.\.\//g, "")}.json`,
						);
						...
						try {
							...
							if (listing) {
								const yearFiles = listing.filter((f) =>
									f.name.endsWith(".json"),
								);
								for (const yearFile of yearFiles) {
									const yearPath = makePath(
										"local/sync",
										group.name,
										yearFile.name,
									);
			...
			if (isMounted.current) {
				ResearchStore.update((s) => {
					s.status = translations.OPTIMIZING_INDEX || "Optimizing index...";
				});
