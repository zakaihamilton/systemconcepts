...
function addParagraphTokens(newIndex: any, fileIndex: any, paragraphs: any) {
	paragraphs.forEach((para: any, paraIndex: any) => {
		const paraTokens = para
			.toLowerCase()
			.split(/[^a-z0-9\u0590-\u05FF]+/)
			.filter(Boolean);
		const uniqueTokens = [...new Set<string>(paraTokens)];
...
function compressTokenRefs(newIndex: any) {
	Object.keys(newIndex.t).forEach((token) => {
		const refs = newIndex.t[token];
		const fileMap = new Map<any, any>();
		...
		for (const f of sortedFiles) {
			compressed.push(-(f + 1));
			const paras = fileMap.get(f).sort((a: any, b: any) => a - b);
...
async function loadSessionsForIndexing(storage: any) {
	const sessions: any = [];
	try {
		...
		for (const group of groups) {
			...
			try {
				...
				if (listing) {
					const yearFiles = listing.filter((f: any) =>
...
export async function buildSearchIndex({
	...
	encodeIndex?: typeof encodeBinaryIndex;
}) {
	...
	const tagsByPath: Record<string, any> = {};
	...
	await Promise.all([
		...
		...sessions.map((session: any) => limit(() => processSession(session))),
	]);
