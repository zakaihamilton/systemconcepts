...
export default function BatchDialog({ open, onClose, tags, loadTags }: any) {
	...
	const handlePreview = () => {
		setCalculating(true);
		try {
			const affectedItems = tags.filter((tag: any) => {
				const matchFilter = !filterKey || tag[filterKey] === filterValue;
				const matchSource =
					tag.hasOwnProperty(sourceKey) &&
					(!sourceValue || tag[sourceKey] === sourceValue);
				return matchFilter && matchSource;
			});

			const files: Record<string, any> = {};
			affectedItems.forEach((tag: any) => {
				if (!files[tag.path]) {
					files[tag.path] = [];
				}
			...
			const changes = Object.entries(files).map(([path, fileTags]) => ({
				...
				items: fileTags.map(
					(t: any) => t.article || t.title || t.name || t._id,
				),
			}));
	...
	const handleUpdate = async () => {
		...
		try {
			...
			const filePaths = Array.from(
				new Set(relevantTags.map((t: any) => t.path)),
			);
			...
			for (const relativePath of filePaths) {
				...
				if (await storage.exists(filePath)) {
					...
					const relevantIds = relevantTags
						.filter((t: any) => t.path === relativePath)
						.map((t: any) => t._id);
					...
					const updateItem = (item: any) => {
						if (relevantIds.includes(item._id)) {
							...
							const matchSource =
								item.hasOwnProperty(sourceKey) &&
								(!sourceValue || item[sourceKey] === sourceValue);
	...
	return (
		<Dialog
			...
			fullWidth
		>
			<DialogTitle>{translations.BATCH_UPDATE_TAGS}</DialogTitle>
					...
					<div className={styles.section}>
						...
						<div className={styles.field}>
							<TextField
								...
								value={destKey}
								onChange={(e: any) => setDestKey(e.target.value)}
							>
								{LibraryTagKeys.map((key) => (
									<MenuItem key={key} value={key}>
										{key}
						...
						<div className={styles.field}>
							<TextField
								...
								value={destValue}
								onChange={(e: any) => setDestValue(e.target.value)}
							/>
						</div>
					</div>
				</div>
...
function PreviewItem({ item, translations }: any) {
