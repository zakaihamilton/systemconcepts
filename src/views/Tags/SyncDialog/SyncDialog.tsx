...
export default function SyncDialog({ open, onClose, tags }: any) {
		...
		try {
			...
			for (const [relativePath, fileTags] of Object.entries(filesToUpdate)) {
				...
				if (await storage.exists(filePath)) {
					...
					const compareItem = (item: any, tag: any) => {
						const itemDiffs = [];
					...
					if (Array.isArray(data)) {
						data.forEach((item) => {
							const tag = fileTags.find((t: any) => t._id === item._id);
							if (tag) {
						...
						const tag = fileTags.find((t: any) => t._id === data._id);
						if (tag) {
			...
			for (const relativePath of filesToUpdate) {
				...
				if (await storage.exists(filePath)) {
					...
					const updateItem = (item: any) => {
						...
						if (itemChange) {
							itemChange.diffs.forEach((diff: any) => {
									...
									<List disablePadding className={dialog.nestedList}>
										{file.changes.map((change: any) => (
											...
											>
												...
												{change.diffs.map((diff: any, i: any) => (
													<Typography
														key={i}
														variant="caption"
														display="block"
