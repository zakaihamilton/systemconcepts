...
export default function Day({
	...
	const items = sessionItems.map((item: any) => {
		const groupName =
				...
				{!!items.length && (
					<div className={styles.container}>
						<div className={styles.indicators}>
							{items.slice(0, 12).map((item: any, index: any) => (
								<Tooltip
									arrow
									title={
									...
									<span>
										<div
											className={clsx(
												styles.dot,
												playingSession &&
													playingSession.name === item.name &&
													playingSession.group === item.group &&
													playingSession.date === item.date &&
													styles.playing,
											)}
