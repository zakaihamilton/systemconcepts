...
export default function DayView({
	...
	const daySessions = sessions.filter((s: any) => s.date === sessionDate);

	// Group sessions
	const groups = daySessions.reduce((acc: any, session: any) => {
	...
	const yearItems = getYearNames(date, yearFormatter, yearStart, yearEnd).map(
		(name, index) => {
			return {
				id: yearStart + index,
				name,
	...
	useToolbar({
		id: "DayView",
		items: toolbarItems,
		depends: [translations, date, lastViewMode],
	});
		...
		<div className={styles.root} {...swipeHandlers}>
			<div className={styles.title}>
				<Tooltip title={translations.WEEK_VIEW}>
					<span onClick={goWeek} className={styles.link}>
						{weekday}
					</span>
				</Tooltip>
				<span className={styles.separator}>, </span>
				<Tooltip title={translations.MONTH_VIEW}>
					<span onClick={goMonth} className={styles.link}>
						{day} {monthDay}
					</span>
				</Tooltip>
				<span className={styles.separator}>, </span>
				<Tooltip title={translations.YEAR_VIEW}>
					<span onClick={goYear} className={styles.link}>
						{year}
					</span>
				</Tooltip>
			</div>
			<div className={styles.list}>
				{items.length ? (
					...
					<div className={styles.empty}>
						{translations.NO_SESSIONS || "No sessions"}
					</div>
				)}
			</div>
		</div>
