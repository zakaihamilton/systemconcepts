...
jest.mock("@components/Widgets/Input", () => (props: any) => (
...
jest.mock(
	"./SessionGroup",
	() =>
		({ group, sessions, playingSession }: any) => (
...
function getWidgetState(id: any) {
	return getToolbarItem(id).element.props.state;
}

function getWidgetItems(id: any) {
...
describe("DayView", () => {
	...
	beforeEach(() => {
		...
		asMock(useDateFormatter).mockImplementation((opts = {}) => ({
			...
			formatWithOrdinal: (date: any) => `${date.getDate()}th`,
		}));
	});
