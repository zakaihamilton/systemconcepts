...
const InnerList = forwardRef<any, any>(({ style, ...rest }: any, ref) => (
	<div
		ref={ref}
		style={{
			...style,
...
export default function TrackRow({
	...
	useEffect(() => {
		if (focusedSessionId && outerRef.current) {
			const index = sessions.findIndex((s: any) => s.id === focusedSessionId);
			if (index !== -1) {
	...
	const RowItem = useCallback(({ index, style, data }: any) => {
		...
		const isPlaying =
			playingSession &&
			playingSession.name === session.name &&
			...
			<div className={styles.header}>
				...
				<div className={styles.badges}>
					...
					<Tooltip
						title={
							showBadges
								? translations.HIDE_GROUP_COUNTERS
								: translations.SHOW_GROUP_COUNTERS
						}
						arrow
					>
						<IconButton
				...
				{safeWidth > 0 && (
					<FixedSizeList
						...
						initialScrollOffset={parseInt(
							sessionStorage.getItem(`track_scroll_${date}`) || "0",
						)}
