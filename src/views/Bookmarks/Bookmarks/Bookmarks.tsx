...
export const BookmarksStore = createStore(BookmarksStoreDefaults);
...
export default function BookmarksPage() {
	...
	const mapper = (item: any) => {
		const breadcrumbPages = getPagesFromHash({
			hash: item.id,
			translations,
			pages,
		});
		...
		return {
			...
			nameWidget: (
				<Row
					...
					onClick={bookmarkClick.bind(null, item)}
					icons={iconWidget}
				>
					{item.name}
				</Row>
			...
			locationWidget: (
				<Breadcrumbs navigateLast={true} items={breadcrumbPages.slice(0, -1)} />
			),
		};
	};

	const statusBar = (
		<StatusBar data={bookmarks} mapper={mapper} store={BookmarksStore} />
	);
