...
jest.mock("@util/locale", () => ({
    useDateFormatter: jest.fn().mockReturnValue({ format: jest.fn().mockReturnValue('Formatted Date') }),
}));
...
describe('Audio Component', () => {
  ...
  it('renders audio player container and sub-components', () => {
    const { getByTestId, getByText } = render(
      <Audio show={true} name="Test Audio" group="testgroup" color="red" date="2021-01-01" />
    );
