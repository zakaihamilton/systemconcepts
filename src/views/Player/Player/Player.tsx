...
export default function PlayerPage({
	...
	useLocalStorage("PlayerStore", PlayerStore, [
		"showSpeed",
		"showSubtitles",
		"showDetails",
	]);
	const [, , groups] = useSessions([], {
	...
	const {
		...
		date = "",
		name = "",
	} = { ...useParentParams(), ...props };
	let components = [prefix, group, year, date + " " + name + (suffix || "")]
		.filter(Boolean)
		.join("/");
	...
	const groupInfo = groups.find((g: any) => g.name === group);
	...
	const color = groups.find((item: any) => item.name === group)?.color;

	const playingSessionName = session && (
		<div className={styles.playingSessionName}>
			<b>{session.group[0].toUpperCase() + session.group.substring(1)}</b>
			<div>{session.date + " " + session.name}</div>
		</div>
	);
	...
	if (isTranscript) {
		...
		mediaStyles.height =
			size.height -
			size.emPixels * baseEmSubtraction -
			speedSliderHeight +
			"px";
	}
	...
	return (
		<div className={styles.root} style={style}>
			...
			<Download
				visible={show && downloadUrl && !isTranscript}
