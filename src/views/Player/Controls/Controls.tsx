...
export default function Controls({
	...
	const pendingResumeRef = useRef<any>(null);
	...
	useEffect(() => {
		...
		return () => {
			...
			listeners.map(({ name, callback }: any) =>
	...
	const handlePosEvent = useCallback(
		(e: any) => {
			...
			let coord = clientX - progressElement.getBoundingClientRect().left;
	...
	useEffect(() => {
		if (
			...
			setMetadata((data: any) => {
				...
				const position = Math.floor(Number(currentTime));
