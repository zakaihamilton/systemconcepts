...
import PlayerPage, { PlayerStore } from './Player';
...
describe('Player View', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useTranslations.mockReturnValue({ SUBTITLES: 'Subtitles', DETAILS: 'Details' });
    useFetchJSON.mockReturnValue([{ path: 'test.mp4' }, false, false]);
    useSessions.mockReturnValue([[], false, []]);
    useParentParams.mockReturnValue({ group: 'test', year: '2021', date: '01-01', name: 'session' });
  ...
  it('renders audio player for audio files', () => {
    const { getByTestId } = render(
      <ContentSize.Provider value={mockSize}>
        <PlayerPage show={true} prefix="sessions" group="test" year="2021" date="01-01" name="session.m4a" />
  ...
  it('renders video player for video files', () => {
    const { getByTestId } = render(
      <ContentSize.Provider value={mockSize}>
        <PlayerPage show={true} prefix="sessions" group="test" year="2021" date="01-01" name="session.mp4" />
      </ContentSize.Provider>
    );
