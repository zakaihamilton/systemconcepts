...
export default function Transcript({ show }: any) {
	...
	const prevName = useMemo(() => {
		if (matches.length > 0 && currentMatchIndex !== -1) {
			...
			if (line) {
				return (
					translations.PREVIOUS_MATCH + " (" + formatTime(line.start) + ")"
				);
			}
		}
	...
	const handleDownload = () => {
		...
		link.href = url;
		let name =
			decodeURIComponent(activeUrl).split("/").pop()?.split("?")[0] ||
			"transcript";
	...
	const highlightText = (text: any, lineIndex: any) => {
		...
		lineMatches.forEach((match: any, i: any) => {
			...
			parts.push(
				<span
					key={i}
					className={`${styles.highlight} ${isCurrent ? styles.currentMatch : ""}`}
				>
	...
	const searchStatusPortal =
		typeof document === "undefined"
			? null
			: document.getElementById("search-status-portal");
	...
	return (
		<div className={styles.root}>
					...
					<div className={styles.searchStatus}>
						...
						<div className={styles.searchMatches}>
							{(translations.MATCHES || "{current} of {total}")
								.replace("{current}", String(currentMatchIndex + 1))
			...
			<Download
				...
				title={translations.DOWNLOAD_TRANSCRIPT}
			/>
			<div className={styles.transcript} ref={scrollRef}>
				{transcript.map((line, index) => {
					...
					return (
						...
						>
							<span className={styles.time}>{formatTime(line.start)}</span>
