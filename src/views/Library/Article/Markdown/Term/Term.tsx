...
const Term = ({ term, entry, search }: any) => {
	const translations = useTranslations();
	const [hover, setHover] = useState(false);
	const [tooltipStyle, setTooltipStyle] = useState<Record<string, any>>({});
	const [bridgeStyle, setBridgeStyle] = useState<Record<string, any>>({});
	...
	React.useLayoutEffect(() => {
		if (hover && tooltipRef.current && containerRef.current && !isMeasured) {
			...
			let newTooltipStyle: Record<string, any> = {};
			let newBridgeStyle: Record<string, any> = {};
						...
						>
							...
							{phaseLabel && (
								<div
									...
									style={{
										display: "inline-block",
										background: phaseColor || undefined,
										color: "#000",
										padding: "2px 6px",
										borderRadius: "4px",
										fontSize: "0.7rem",
