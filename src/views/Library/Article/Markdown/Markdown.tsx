...
const rehypeArticleEnrichment: any = () => {
	return (tree: any) => {
		let paragraphIndex = 0;
		const paragraphs: any = [];

		const visitAndSplit: any = (nodes: any) => {
			const newNodes: any = [];
...
export default React.memo(function Markdown({
	...
	const paragraphsRef = useRef<Record<number, any>>({});
	...
	const handleParagraphZoom = useCallback((children: any, number: any) => {
	...
	const markdownComponents = useMemo(() => {
		...
		const getSpokenText = (text: any) => {
			if (!text) return text;
			return text.replace(
				termPattern,
				(match: any, _capture: any, offset: any, string: any) => {
					if (shouldSkipGlossaryTerm(match, string, offset)) {
						return match;
					}
					const lowerMatch = match.toLowerCase();
					const entry = (glossary as Record<string, any>)[lowerMatch];
					if (entry && entry.en) {
						return entry.en;
					}
					return match;
		...
		const HeaderRenderer = (tag: any) => {
			const Header = ({ node, children }: any) => {
		...
		const ParagraphRenderer = ({ node, children }: any) => {
		...
		const BlockRenderer = (Tag: any) => {
			const Renderer = ({ node, children }: any) => {
				const paragraphIndex = node?.properties?.dataParagraphIndex;
