...
function isStartOfSentence(text: any, start: any) {
 ...
 */
export function shouldSkipGlossaryTerm(term: any, text: any, start: any) {
...
export function replaceAbbreviations(text: any) {
...
export function scanForTerms(text: any) {
	if (!text || typeof text !== "string") return [];

	const foundTerms = new Map<any, any>();
	...
	return Array.from(foundTerms.entries())
		.map(([key, entry]) => ({
			term: key,
			...entry,
			paragraphs: Array.from(entry.paragraphs as Set<number>).sort(
				(a, b) => a - b,
			),
		}))
...
export const getStyleInfo = (style: any) => {
