	...
	() => (props: any) =>
		props.open ? (
			<div data-testid="terms-dialog">
				...
				{props.terms?.map((t: any) => t.term).join(",")}
			</div>
		) : null,
);

describe("Article Component", () => {
	let setShowAbbreviations: any;
	let setHideSquareBrackets: any;
	let toolbarItems: any;
	let handleScrollUpdate: any;
	...
	it("registers toolbar items including search matches and navigation", () => {
		...
		const ids: any = toolbarItems.map((i: any) => i.id);
		...
		toolbarItems.find((i: any) => i.id === "prevArticle").onClick();
		toolbarItems.find((i: any) => i.id === "nextArticle").onClick();
	...
	it("includes admin edit actions when authorized", () => {
		...
		expect(toolbarItems.map((i: any) => i.id)).toEqual(
			expect.arrayContaining(["editTags", "editArticle"]),
		);
		toolbarItems.find((i: any) => i.id === "editTags").onClick();
	...
	it("toggles abbreviations and square brackets from toolbar", () => {
		...
		toolbarItems.find((i: any) => i.id === "toggleAbbreviations").onClick();
		expect(setShowAbbreviations).toHaveBeenCalled();
		toolbarItems.find((i: any) => i.id === "toggleSquareBrackets").onClick();
	...
	it("jumps to a paragraph and page via JumpDialog", () => {
		...
		Object.defineProperty(root, "querySelector", {
			value: (sel: any) => container.querySelector(sel),
		});
		Object.defineProperty(root, "querySelectorAll", {
			value: (sel: any) => container.querySelectorAll(sel),
		});
	...
	it("exports markdown when not showing markdown", async () => {
		...
		toolbarItems.find((i: any) => i.id === "toggleMarkdown").onClick();
		// After toggle, showMarkdown becomes false; re-get items by re-render
		await waitFor(() => {
			const exportItem: any = toolbarItems.find((i: any) => i.id === "export");
	...
	it("exports formatted markdown when plain text mode is enabled", () => {
		...
		act(() => {
			toolbarItems.find((i: any) => i.id === "export").onClick();
		});
	...
	it("shows admin edit controls when roleAuth allows", () => {
		...
		expect(toolbarItems.some((item: any) => item?.id === "editArticle")).toBe(
			true,
		);
	});
});
