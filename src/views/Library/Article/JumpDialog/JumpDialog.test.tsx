...
jest.mock("@ui/DialogTitle", () => ({ children }: any) => <h2>{children}</h2>);
jest.mock("@ui/DialogContent", () => ({ children }: any) => (
	<div>{children}</div>
));
jest.mock("@ui/DialogActions", () => ({ children }: any) => (
	<div>{children}</div>
));
jest.mock("@ui/Tabs", () => ({ children, onChange, value }: any) => (
...
jest.mock("@ui/Tab", () => ({ label }: any) => <span>{label}</span>);
jest.mock(
	"@ui/TextField",
	() =>
		({ label, value, onChange, onKeyDown, inputRef, id }: any) => (
			<input
				aria-label={label}
				id={id}
				value={value}
...
jest.mock("@ui/Button", () => ({ children, onClick }: any) => (
	<button type="button" onClick={onClick}>
		{children}
	</button>
));
jest.mock("@ui/Box", () => ({ children }: any) => <div>{children}</div>);
