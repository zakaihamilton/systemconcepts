...
export default function Player({
	...
	onParagraphChange,
	selectedTag,
	currentParagraphIndex: externalParagraphIndex,
}: any) {
	...
	useEffect(() => {
		...
		if (paragraphs.length > 0 && externalParagraphIndex !== undefined) {
			const index = paragraphs.findIndex(
				(p) => p.index === externalParagraphIndex,
			);
			if (index !== -1) {
	...
	const [voiceMenuAnchor, setVoiceMenuAnchor] = useState<any>(null);
	...
	const prevVoiceRef = useRef<any>(null);
					...
					if (!preferredVoice) {
						...
						preferredVoice =
							availableVoices.find(
								(v: any) => v.name === "Google UK English Male",
							) ||
							availableVoices.find(
								(v: any) => v.name === "Google UK English",
							) ||
							availableVoices.find((v: any) => v.lang.startsWith("en-")) ||
							availableVoices[0];
					}
	...
	useEffect(() => {
		...
		const extractParagraphs = () => {
			...
			if (selectedTag) {
				...
				if (titleText) {
					const numberPrefix = selectedTag.number
						? `Number ${selectedTag.number}. `
						: "";
					result.push({
			...
			const pElements = container.querySelectorAll<HTMLElement>(
				"[data-paragraph-index]",
			);
			const extracted = Array.from(pElements).map((el, index) => {
				const text =
					el.getAttribute("data-paragraph-text") ||
					el.textContent?.trim() ||
					"";
				return {
					element: el,
					text: text,
					index:
						parseInt(el.getAttribute("data-paragraph-index") || "0", 10) ||
						index,
					isVirtual: false,
				};
			});
	...
	const speakParagraphRef = useRef<any>(null);
	const speakParagraph = useCallback(
		(index: any, autoPlay = true) => {
	...
	const handlePlay = useCallback(() => {
		...
		if (synthRef.current.paused || synthRef.current.speaking) {
			...
			const startIndex =
				currentParagraphIndex !== -1 ? currentParagraphIndex : 0;
	...
	// Voice menu handlers
	const handleVoiceMenuOpen = (event: any) => {
