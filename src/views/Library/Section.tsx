...
export function getLibrarySection({ id, path, translations }: any) {
	...
	if (tags && tags.length > 0) {
		...
		if (bestTag && bestFieldKey) {
			...
			description =
				bestFieldKey.charAt(0).toUpperCase() + bestFieldKey.slice(1);
		}
	}
	...
	if (
		description &&
		name &&
		name.toLowerCase().startsWith(description.toLowerCase())
	) {
