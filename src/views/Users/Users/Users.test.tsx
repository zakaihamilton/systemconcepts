...
import Users, { UsersStore } from "./index";
...
jest.mock("@widgets/Row", () => ({ children }: any) => <div>{children}</div>);
...
describe("Users View", () => {
	...
	it("maps users and navigates on click when not selecting", () => {
		render(<Users />);
		const mapped: any = lastTableProps.mapper(users[0]);
	...
	it("filters by role via role column click", () => {
		render(<Users />);
		const roleCol: any = lastTableProps.columns.find(
			(c: any) => c && c.id === "roleWidget",
		);
