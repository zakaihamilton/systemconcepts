...
export const UsersStore = createStore(UsersStoreDefaults);
...
export default function Users() {
	...
	const columns = [
		...
		},
		{
			...
			selected: () => roleFilter,
			onSelectable: (item: any) => item.role,
			onClick: (item: any) =>
				UsersStore.update((s) => {
					if (s.roleFilter) {
						s.roleFilter = "";
	...
	const mapper = (item: any) => {
		...
		const href =
			"#users/" + toPath("user/" + id + "?name=" + firstName + " " + lastName);
		return {
			...item,
			nameWidget: (
				<Row
					href={href}
					onClick={userClick.bind(null, item)}
					icons={iconWidget}
				>
					{labelName}
				</Row>
	...
	const statusBar = (
		<StatusBar data={data} mapper={mapper} store={UsersStore} />
	);
