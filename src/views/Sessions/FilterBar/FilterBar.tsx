...
export default React.memo(function FilterBar({ hideYears = false }: any) {
		...
		() => [
			{
				...
				header: true,
				highlight: ["audio", "video", "image", "overview", "ai"].some((id) =>
					typeFilter.includes(id),
				),
				expanded: ["audio", "video", "image", "overview", "ai"].some((id) =>
					typeFilter.includes(id),
				),
				items: [
			...
			},
			{
				...
				header: true,
				highlight:
					typeFilter.includes("with_thumbnail") ||
					typeFilter.includes("without_thumbnail"),
				expanded:
					typeFilter.includes("with_thumbnail") ||
					typeFilter.includes("without_thumbnail"),
				items: [
					...
					{
						id: "with_thumbnail",
						name: translations.WITH_THUMBNAIL,
						radio: true,
					},
					{
						id: "without_thumbnail",
						name: translations.WITHOUT_THUMBNAIL,
						radio: true,
					},
				],
			},
			{
				...
				header: true,
				highlight:
					typeFilter.includes("with_summary") ||
					typeFilter.includes("without_summary"),
				expanded:
					typeFilter.includes("with_summary") ||
					typeFilter.includes("without_summary"),
				items: [
					...
					{
						id: "without_summary",
						name: translations.WITHOUT_SUMMARY,
						radio: true,
					},
				],
			},
			{
				...
				header: true,
				highlight:
					typeFilter.includes("with_tags") ||
					typeFilter.includes("without_tags"),
				expanded:
					typeFilter.includes("with_tags") ||
					typeFilter.includes("without_tags"),
				items: [
			...
			},
			{
				...
				header: true,
				highlight:
					typeFilter.includes("with_duration") ||
					typeFilter.includes("without_duration"),
				expanded:
					typeFilter.includes("with_duration") ||
					typeFilter.includes("without_duration"),
				items: [
					...
					{
						id: "with_duration",
						name: translations.WITH_DURATION,
						radio: true,
					},
					{
						id: "without_duration",
						name: translations.WITHOUT_DURATION,
						radio: true,
					},
				],
			},
			{
				...
				header: true,
				highlight:
					typeFilter.includes("with_position") ||
					typeFilter.includes("without_position"),
				expanded:
					typeFilter.includes("with_position") ||
					typeFilter.includes("without_position"),
				items: [
					...
					{
						id: "with_position",
						name: translations.WITH_POSITION,
						radio: true,
					},
					{
						id: "without_position",
						name: translations.WITHOUT_POSITION,
						radio: true,
					},
				],
			},
			{
				...
				header: true,
				highlight:
					typeFilter.includes("with_english") ||
					typeFilter.includes("with_hebrew"),
				expanded:
					typeFilter.includes("with_english") ||
					typeFilter.includes("with_hebrew"),
				items: [
				...
				onClick: () => {
					SessionsStore.update((s) => {
						if (s.groupFilter.includes(group.name)) {
							s.groupFilter = s.groupFilter.filter(
	...
	const typeMenuItems = useMemo(() => {
		const mapItems = (list: any, insideHeader = false) => {
			return (list || []).map((type: any) => ({
				...
				header: type.header,
				highlight: insideHeader
					? false
					: type.header
						? type.highlight || false
						: undefined,
				radio:
					(type.id === "thumbnails_all" &&
						!typeFilter.includes("with_thumbnail") &&
						!typeFilter.includes("without_thumbnail")) ||
					(type.id === "summaries_all" &&
						!typeFilter.includes("with_summary") &&
						!typeFilter.includes("without_summary")) ||
					(type.id === "tags_all" &&
						!typeFilter.includes("with_tags") &&
						!typeFilter.includes("without_tags")) ||
					(type.id === "position_all" &&
						!typeFilter.includes("with_position") &&
						!typeFilter.includes("without_position")) ||
					(type.id === "duration_all" &&
						!typeFilter.includes("with_duration") &&
						!typeFilter.includes("without_duration")) ||
					(type.id === "languages_all" &&
						!typeFilter.includes("with_english") &&
						!typeFilter.includes("with_hebrew")) ||
				...
				onClick: (event: any) => {
					...
					SessionsStore.update((s) => {
						const allRadios: Record<string, string[]> = {
						...
						if (allRadios[type.id]) {
							s.typeFilter = s.typeFilter.filter(
								(t) => !allRadios[type.id].includes(t),
							);
							return;
						} else if (type.radio) {
							const otherRadios: Record<string, string> = {
			...
			<div className={styles.container}>
				...
				<Menu items={typeMenuItems} selected={typeFilter}>
					<button
						className={clsx(
							...
							typeFilter.length > 0 && styles.active,
						)}
					>
				...
				{!hideYears && (
					<Menu items={yearMenuItems} selected={yearFilter}>
						<button
							className={clsx(
								styles.dropdownButton,
								yearFilter.length > 0 && styles.active,
							)}
						>
							<ArrowDropDownIcon className={styles.arrow} />
							<div className={styles.labelContent}>
				...
				<Menu items={groupMenuItems} selected={groupFilter}>
					<button
						className={clsx(
							styles.dropdownButton,
							groupFilter.length > 0 && styles.active,
						)}
					>
						<ArrowDropDownIcon className={styles.arrow} />
						<div className={styles.labelContent}>
