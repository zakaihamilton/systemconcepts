...
jest.mock("@util/sessions", () => ({
    useSessions: jest.fn(),
    SessionsStore: {
        useState: jest.fn().mockImplementation(selector => selector({
            viewMode: 'list',
            groupFilter: [],
            typeFilter: [],
            yearFilter: [],
...
jest.mock("@views/Sessions/FilterBar", () => () => <div data-testid="filter-bar" />);
jest.mock("@widgets/StatusBar", () => () => <div data-testid="status-bar" />);
jest.mock("@widgets/Image", () => () => <div data-testid="image" />);
jest.mock("@widgets/SessionIcon", () => () => <div data-testid="session-icon" />);
...
describe('Sessions View', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useTranslations.mockReturnValue({ SESSIONS: 'Sessions', THUMBNAIL: 'Thumbnail', NAME: 'Name', DATE: 'Date', TYPE: 'Type', DURATION: 'Duration', GROUP: 'Group' });
