...
function useDelayedLoading(loading: any) {
...
function getSessionKey({ group, date, name }: any) {
	return `${group || ""}::${date || ""}::${name || ""}`;
}

function getSessionCardKey(session: any, index: any) {
	return `${getSessionKey(session)}::${session.id || session.key || index}::${index}`;
}

function getSessionPath({ group, year, date, name }: any) {
	return `session?group=${group}&year=${year}&date=${date}&name=${encodeURIComponent(name)}`;
}

function getSessionImagePath(session: any) {
...
function SessionSection({ children, title, href, onClick }: any) {
	return (
...
function SessionSkeletons({ count = SESSION_LIMIT }: any) {
	return (
		<div
...
function SessionContent({ translations }: any) {
	...
	const needsSessionReload = SyncActiveStore.useState(
		(state: any) => state.needsSessionReload,
	);
	const updateSessionsBusy = UpdateSessionsStore.useState(
		(state: any) => state.busy,
	);
	...
	useEffect(() => {
		...
		SessionsStore.update((state: any) => {
	...
	const openScheduleView = (viewMode: any) => {
		...
		ScheduleStore.update((state: any) => {
	...
	const sessionsByKey = new Map<string, any>(
		(sessions || []).map((session: any) => [getSessionKey(session), session]),
	);
	const seenContinueWatchingSessions = new Set<any>();
				...
				>
					{showContinueWatchingSkeletons ? (
						...
						<div className={styles.sessionGrid}>
							{continueWatching.map((session: any, index: any) => (
								<SessionCard
...
function QuickAccess({ pages, translations }: any) {
	const appItems = [
		...pages
			.filter(
				(page: any) =>
		...
		...TRAILING_QUICK_ACCESS_PAGE_IDS.map((id) =>
			pages.find((page: any) => page.id === id),
		).filter(Boolean),
	];
...
function SignInPrompt({ translations }: any) {
	return (
		<section
...
function SyncPrompt({ translations, onStart, busy, percentage }: any) {
	return (
