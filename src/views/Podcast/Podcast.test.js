...
describe('Podcast View', () => {
  ...
  it('renders podcast feed when signed in and user has rssToken', async () => {
    ...
    fetchJSON.mockResolvedValue({ id: 'testuser', rssToken: 'token123', role: 'user' });
    
    const { getByText } = render(<Podcast />);
    
    await waitFor(() => {
        expect(getByText('Podcast')).toBeInTheDocument();
        expect(getByText('http://localhost/api/rss?id=testuser&token=token123')).toBeInTheDocument();
    });
  });
  ...
  it('copies URL to clipboard when Copy URL is clicked', async () => {
    ...
    await waitFor(() => {
        ...
        expect(mockWriteText).toHaveBeenCalledWith('http://localhost/api/rss?id=testuser&token=token123');
    });
  });
});
