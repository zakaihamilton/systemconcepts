...
export default function Account({ redirect }: any) {
	...
	const changeRemember = (event: any) => setRemember(event.target.checked);
	const changeCreatePasskey = (event: any) =>
		setCreatePasskey(event.target.checked);
	...
	const onRegisterPasskey = async (targetId?: string) => {
		...
		try {
			...
			setError(translations[String(err)] || String(err));
		} finally {
			setProgress(false);
		}
	};

	const onDeletePasskey = async (credentialId: any) => {
		if (
			!window.confirm(
