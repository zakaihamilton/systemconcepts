...
jest.mock("@views/Schedule/Schedule", () => ({ ScheduleStore: { useState: jest.fn() } }));
jest.mock("@util/sessions", () => ({ SessionsStore: { useState: jest.fn() } }));
jest.mock("@views/Library/Store", () => ({ LibraryStore: { useState: jest.fn() } }));
jest.mock("@components/Breadcrumbs", () => (props) => <div data-testid="breadcrumbs" data-props={JSON.stringify(props)} />);
