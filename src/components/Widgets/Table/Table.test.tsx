...
jest.mock("@widgets/Message", () => ({ label, Icon }: any) => (
	<div data-testid="message">{label}</div>
));
jest.mock("@widgets/Tooltip", () => ({ children, title }: any) => (
	<span title={title}>{children}</span>
));
jest.mock("./Row", () => (props: any) => (
	<tr
...
jest.mock(
	"./Navigator",
	() =>
		({ pageIndex, setPageIndex, pageCount }: any) => (
...
jest.mock("./Error", () => ({ error }: any) => (
	<div data-testid="table-error">{String(error)}</div>
));
jest.mock(
	"./TableColumn",
	() =>
		({ item, createSortHandler, orderBy }: any) => {
			const sortId =
				typeof item.sortable === "string" ? item.sortable : item.id;
			return (
				<th data-testid="table-column" data-id={item.id}>
					...
					{orderBy === sortId ? (
						<span data-testid={`sorted-${item.id}`} />
					) : null}
				</th>
			);
		},
);
jest.mock("./ListColumns", () => ({ columns, onSort, orderBy }: any) => (
...
jest.mock("@components/Virtualized/FixedSizeGrid", () => {
	...
	return React.forwardRef(function MockFixedSizeGrid(
		{ children: Child, rowCount, columnCount, itemData, onScroll }: any,
		ref: any,
	) {
...
describe("Table Widget", () => {
		...
		renderTable(
			{
				...
				rowClassName,
				rowClick,
				selectedRow: (item: any) => item.id === 2,
				treeGroup: (items: any) => items,
			},
			{ viewMode: "tree" },
		);
		expect(screen.getByTestId("fixed-list")).toBeInTheDocument();
		...
		renderTable(
			{
				...
				cellWidth: "10em",
				cellHeight: "10em",
				selectedRow: (item: any) => item.id === 1,
				rowClick: jest.fn(),
				rowClassName: () => "g",
			},
			{ viewMode: "grid" },
	...
	it("applies custom filter and mapper", () => {
		renderTable({
			...
			mapper: (item: any) => ({ ...item, name: item.name.toUpperCase() }),
		});
		expect(screen.getAllByTestId("table-row")).toHaveLength(2);
