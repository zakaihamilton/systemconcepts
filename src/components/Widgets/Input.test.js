...
describe('Input Widget', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useTranslations.mockReturnValue({ SHOW_PASSWORD: 'Show', HIDE_PASSWORD: 'Hide' });
  });
  ...
  it('calls setValue and onValidate when text changes', () => {
    ...
    const { getByLabelText, getByText } = render(
      <InputWidget label="Username" state={['', setValue]} validate={true} onValidate={onValidate} />
    );
    const input = getByLabelText('Username');
  ...
  it('toggles password visibility', () => {
    const setValue = jest.fn();
    const { getByLabelText } = render(
      <InputWidget label="Password" type="password" state={['pass', setValue]} />
    );
