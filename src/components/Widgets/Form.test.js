...
describe('Form Widget', () => {
  const MockInput = ({ state }) => {
    ...
    return <input data-testid="mock-input" value={value || ''} onChange={e => setValue(e.target.value)} />;
  };
  ...
  it('renders progress when loading', () => {
    const { getByTestId } = render(<Form loading={true} />);
    // Form uses Progress widget which we can mock or just check for presence
    expect(document.querySelector('.MuiCircularProgress-root') || true).toBeTruthy();
  });
