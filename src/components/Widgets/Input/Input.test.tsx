...
import InputWidget, { arrayToMenuItems } from "./index";

jest.mock("@util/domain/translations");
jest.mock("@ui/Autocomplete", () => ({
	__esModule: true,
	default: ({ options, value, onChange, renderInput, className }: any) => (
	...
	it("renders select items via arrayToMenuItems and custom mapping/render", () => {
		...
		rerender(
			<InputWidget
				...
				mapping={(item: any) => ({ ...item, name: item.name.toUpperCase() })}
				render={(items: any) =>
					items.map((item: any) => <option key={item.id}>{item.name}</option>)
				}
			/>,
		);
	...
	it("uses custom renderValue for multiple select", () => {
		...
		render(
			<InputWidget
				...
				renderValue={(selected: any) => `picked:${selected.length}`}
				items={[
					{ id: "one", name: "One" },
					{ id: "two", name: "Two" },
				]}
