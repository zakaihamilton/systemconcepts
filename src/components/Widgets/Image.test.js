...
describe('Image Widget', () => {
  ...
  it('renders progress while loading from external source', () => {
    ...
    const { getByTestId } = render(<ImageWidget path="wasabi/test.png" showProgress={true} />);
    expect(getByTestId('progress')).toBeInTheDocument();
  });

  it('renders image when path is provided', async () => {
    useFetchJSON.mockReturnValue([{ path: 'https://example.com/test.png' }, false, false]);
    const { getByRole } = render(<ImageWidget path="https://example.com/test.png" alt="Test Image" />);
    await waitFor(() => {
        expect(getByRole('img')).toHaveAttribute('src', 'https://example.com/test.png');
    });
  });
