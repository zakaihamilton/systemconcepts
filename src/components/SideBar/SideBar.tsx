...
export default function SideBar() {
	...
	const isLibraryActive = !!activePages.find(
		(page: any) => page.id === "library" && page.custom,
	);
	...
	const selected = (id: any) => {
		return !!activePages.find(
			(page: any) => page.id === id && !page.sectionIndex,
		);
	};
	...
	const sidebarPages = pages.filter(
		(page: any) => page.sidebar && !page.category,
	);

	// Separate Library from other apps
	const apps = sidebarPages.filter(
		(page: any) => page.apps && page.id !== "library",
	);
	const library = sidebarPages.find((page: any) => page.id === "library");
	const other = sidebarPages.filter((page: any) => !page.apps);

	const items = [
		...apps.map((item: any, _index: any) => ({
			...item,
			divider: false,
		})),
	]
	...
	const otherItems = other.map((item: any) => {
		let target = item.path || item.id;
		if (item.id === "account") {
			const currentPath =
				hash && (hash.startsWith("#") ? hash.substring(1) : hash);
	...
	if (bookmarks && bookmarks.length) {
		otherItems.push({
			...
			items: [
				...pages
					.filter((page: any) => page.sidebar && page.category === "bookmarks")
					.map((item: any) => {
	...
	const handleLibraryClick = (isOpen: any) => {
		MainStore.update((s) => {
