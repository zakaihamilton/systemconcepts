...
export default function LibraryTree({ closeDrawer, isMobile }: any) {
	...
	const tree = useMemo<LibraryTreeNode[]>(() => {
		...
		if (debouncedFilterText) {
			...
			const keysToSearch = [
				...LibraryTagKeys.filter((k) => k !== "author"),
				"number",
			];
			const terms = lowerFilter.split(/\s+/).filter(Boolean);

			filteredTags = tags.filter((tag) => {
				const values = keysToSearch
					.map((key) => tag[key])
		...
		for (const tag of filteredTags) {
			...
			levels.forEach((levelItem, index) => {
				...
				const node: LibraryTreeNode =
					existingNode ||
					(() => {
						const Icon = (LibraryIcons as Record<string, any>)[type];
						return {
							id,
							name,
							type,
	...
	const handleToggle = useCallback(
		(nodeId: any, isExpanding: any) => {
			if (isExpanding) {
				const siblingIds = tree
					.map((node) => node.id)
					.filter((id) => id !== nodeId);
				LibraryStore.update((s) => {
