...
export default function Autocomplete({
	...
	onInputChange,
	filterOptions,
	isOptionEqualToValue = (option: any, selected: any) => option === selected,
	...
	const setCurrentInputValue = (event: any, nextValue: any) => {
		...
		const nextValue = multiple
			? [
					...selectedValues.filter(
						(selected: any) => !isOptionEqualToValue(option, selected),
					),
					option,
				]
			: option;
		...
		InputProps: {
			...
			startAdornment:
				renderValue && multiple
					? renderValue(selectedValues, ({ index }: any) => ({
							key: `${getOptionLabel(selectedValues[index])}-${index}`,
							onDelete: () =>
								onChange?.(
									null,
									selectedValues.filter(
										(_: any, selectedIndex: any) => selectedIndex !== index,
									),
								),
						}))
					: undefined,
	...
	return (
		...
		>
			...
			{open && filtered.length > 0 && (
				...
				>
					{filtered.map((option: any, index: any) => {
						const optionProps = {
