	...
	it("supports controlled multi-select object options", () => {
		...
		render(
			<Autocomplete
				...
				filterOptions={(items: any, { inputValue }: any) =>
	...
	it("supports uncontrolled single select with default filter", () => {
		...
		render(
			<Autocomplete
				...
				onChange={onChange}
				renderInput={({ InputProps, value, onChange: onInput }: any) => (
					<input
						aria-label="Fruit"
						value={value}
						onChange={onInput}
	...
	it("dedupes multiple selections and deletes via renderValue", () => {
		...
		render(
			<Autocomplete
				...
				renderValue={(selected: any, getItemProps: any) =>
					selected.map((opt: any, index: any) => {
