...
describe('Sync Component', () => {
  beforeEach(() => {
    ...
    SyncActiveStore.useState.mockReturnValue({ personalSyncBusy: false, personalSyncError: null });
  });

  it('renders children within provider', () => {
    const { getByText } = render(<Sync><div>Test Child</div></Sync>);
    expect(getByText('Test Child')).toBeInTheDocument();
  });

  it('registers toolbar item', () => {
    render(<Sync><div>Test</div></Sync>);
