...
export const SyncContext = createContext<any>(undefined);

import { SyncActiveStore, UpdateSessionsStore } from "@sync/syncState";

export default function Sync({ children }: any) {
	const translations = useTranslations();
	const { sync, busy, error, duration, changed, percentage, phase } =
		useSyncFeature();
	...
	const getSyncLabel = () => {
		...
		if (phase === "personal" || personalSyncBusy)
			return translations.SYNCING_PERSONAL;
	...
	const name = (
		<span>
			...
			{!error &&
				!personalSyncError &&
				(isBusy ? getSyncLabel() : translations.SYNC)}
	...
	return (
		<SyncContext.Provider value={syncContext}>{children}</SyncContext.Provider>
	);
}
