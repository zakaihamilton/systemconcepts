...
const FixedSizeList = forwardRef<any, any>(
	(
		{
			children: Component,
			className,
		...
		const internalOuterRef = useRef<any>(null);
		...
		useImperativeHandle(ref, () => ({
			...
			scrollToItem: (index: any, align = "auto") => {
				if (outerRef.current) {
		...
		const stopIndex = Math.min(
			itemCount - 1,
			Math.ceil((scrollOffset + viewSize) / itemSize) + overscanCount,
		);
		...
		return (
			<div
				...
				{...props}
			>
				<InnerElement style={containerStyle}>{items}</InnerElement>
			</div>
		);
	},
);
