...
export default function AudioControls({ playerRef, metadata, setMetadata, path = "", group = "", year = "", name = "" }) {
    ...
    const rewind = () => {
        let position = playerRef.currentTime;
    ...
    const fastforward = () => {
        let position = playerRef.position;
    ...
    useEffect(() => {
        if (playerRef.currentTime && !isNaN(playerRef.currentTime)) {
            setMetadata(data => {
                if (data) {
                    data.position = parseInt(playerRef.currentTime);
                }
                return data;
            });
        }
    ...
    const volumeId = Object.keys(volumeItems).find(volumeId => rateItems[volumeId] === volume);
