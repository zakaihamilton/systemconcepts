...
export async function getCluster({ url = process.env.MONGO_URL }: any) {
	if (!url) {
		throw "Empty URI";
	}
	const unlock = await lockMutex({ id: "mongo" });
	try {
		let cluster: any = _clusters[url];
		if (!cluster) {
...
export async function listCollections({ ...params }: any) {
...
export async function insertRecord({ record, ...params }: any) {
	const collection = await getCollection(params);
	await collection.insertOne(record);
}

export async function findRecord({ query, fields, ...params }: any) {
	const collection = await getCollection(params);
	return await collection.findOne(query, fields);
}

export async function deleteRecord({ query, ...params }: any) {
	const collection = await getCollection(params);
	await collection.deleteOne(query);
}

export async function replaceRecord({ query, record, ...params }: any) {
	const collection = await getCollection(params);
...
export async function bulkWrite({ operations, ordered, ...params }: any) {
	const collection = await getCollection(params);
...
export async function handleRequest({
	...
	const getHeader = (name: any) => headers[name] || headers[name.toLowerCase()];
	const parseLimit = (value: any, fallback: any) => {
		const parsed = Number.parseInt(value || "", 10);
		const safe = Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
		return Math.min(safe, MAX_QUERY_LIMIT);
	};
	const parseSkip = (value: any) => {
		const parsed = Number.parseInt(value || "", 10);
