...
import { GroupsStore, useGroups } from "@util/groups";
...
export function useSessions(depends = [], options = {}) {
		...
		async (syncCounter) => {
			...
			try {
				...
				for (let i = 0; i < nonBundledGroups.length; i += CHUNK_SIZE) {
					...
					const chunkResults = await Promise.all(
						chunk.map(async (group) => {
							...
							let isMerged =
								filesManifest &&
								filesManifest.some((f) => f.path === mergedPath);
							if (!filesManifest) {
				...
				for (let i = 0; i < tasks.length; i += YEAR_CHUNK_SIZE) {
					...
					const chunkResults = await Promise.all(
						chunk.map(async ({ group, year, sessions }) => {
							try {
	...
	const groupsItems = useMemo(() => {
		return groups.map((group) => {
			const metadata =
				(groupMetadata || []).find((item) => item.name === group.name) || {};
			return {
				id: group.name,
				icon:
