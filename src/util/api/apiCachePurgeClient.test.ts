...
describe("purgeApiCacheFromStorage", () => {
	...
	it("swallows a 'no such key' style error and returns 0", async () => {
		asMock(storage.exists).mockResolvedValue(true);
		asMock(storage.deleteFolder).mockRejectedValue(new Error("No Such Key"));
		await expect(purgeApiCacheFromStorage()).resolves.toBe(0);
	});

	it("swallows an ENOENT error and returns 0", async () => {
		asMock(storage.exists).mockResolvedValue(true);
		asMock(storage.deleteFolder).mockRejectedValue(
			new Error("ENOENT: missing"),
		);
		await expect(purgeApiCacheFromStorage()).resolves.toBe(0);
	});
	...
	it("rethrows errors without a message", async () => {
		asMock(storage.exists).mockResolvedValue(true);
		asMock(storage.deleteFolder).mockRejectedValue("plain string error");
