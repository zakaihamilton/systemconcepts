export function makePath(...components: any[]) {
...
export function isBinaryFile(path: any) {
	path = makePath(path);
...
export function isCompressedJSONFile(path: any) {
	path = makePath(path);
	return path.endsWith(".json.gz");
}

export function isMediaFile(path: any) {
	path = makePath(path);
...
export function isVideoFile(path: any) {
	path = makePath(path);
...
export function isAudioFile(path: any) {
	path = makePath(path);
...
export function isImageFile(path: any) {
	path = makePath(path);
...
export function getImageMimeType(path: any) {
...
export function isSubtitleFile(path: any) {
	path = makePath(path);
...
export function isSummaryFile(path: any) {
	path = makePath(path);
...
export function isTagsFile(path: any) {
	path = makePath(path);
...
export function isDurationFile(path: any) {
	path = makePath(path);
...
export function fileExtension(path: any) {
	path = makePath(path);
	const components = path.split(".");
	return components[components.length - 1];
}

export function fileTitle(path: any) {
	path = makePath(path);
	const last = path.split("/").pop();
	const components = last.split(".");
	const extension = components[components.length - 1] || "";
	const extensionLength = components.length > 1 ? extension.length + 1 : 0;
	const result = extensionLength
		? last.slice(0, last.length - extensionLength)
		: last;
	return result;
}

export function fileName(path: any) {
	path = makePath(path);
	const last = path.split("/").pop();
	return last;
}
export function fileFolder(path: any) {
	path = makePath(path);
	const components = path.split("/");
