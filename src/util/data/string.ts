...
export function abbreviateSize(number: any) {
	var tier = (Math.log10(number) / 3) | 0;
...
export function isRTL(string: any) {
	var rtlRegex = /[\u0591-\u07FF]/;
	return rtlRegex.test(string);
}

export function formatDuration(duration: any, includeHours = false) {
	if (
		duration === undefined ||
		duration === null ||
		isNaN(duration) ||
	...
	const hoursString =
		hours || includeHours ? (hours < 10 ? "0" + hours : hours) + ":" : "";
...
export function normalizeContent(text: any) {
 ...
 */
export function preprocessMarkdown(content: any) {
