...
function writeVarInt(value: any, buffer: any, offset: any) {
	let written = 0;
	while (value > 0x7f) {
...
function writeSignedVarInt(value: any, buffer: any, offset: any) {
	// ZigZag encoding: (n << 1) ^ (n >> 31)
	const encoded = (value << 1) ^ (value >> 31);
	return writeVarInt(encoded >>> 0, buffer, offset);
}

function readSignedVarInt(buffer: any, offset: any) {
...
function signedVarIntSize(value: any) {
	const encoded = (value << 1) ^ (value >> 31);
 ...
 */
export function encodeBinaryIndex(indexData: any) {
	// Build string table from all strings
	const stringMap = new Map<any, any>();
	const strings: any = [];

	const addString = (str: any) => {
	...
	// Add file IDs
	const fileIdIndices = indexData.f.map((id: any) => addString(id));

	// Add paragraphs and compute document structure (only for v4 and below)
	const docStructure = [];
	if (indexData.d) {
		const fileIndices = Object.keys(indexData.d)
			.map((k) => parseInt(k, 10))
			.sort((a, b) => a - b);
		for (const fileIdx of fileIndices) {
			const paragraphs = indexData.d[fileIdx];
			const paraIndices = paragraphs.map((p: any) => addString(p));
			docStructure.push({ fileIdx, paraIndices });
		}
	}

	// Add token strings
	const tokenEntries = Object.entries(indexData.t) as Array<[string, number[]]>;
	...
	const encodedStrings: any = strings.map((s: any) => encoder.encode(s));
 ...
 */
export function decodeBinaryIndex(compressedData: any) {
