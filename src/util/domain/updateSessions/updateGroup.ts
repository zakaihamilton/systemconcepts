...
function groupFilesBySessionId(files: any, yearName: any) {
...
function getListingSessionFingerprints(yearItems: any, yearName: any) {
...
function buildMetadataLookup(map: any) {
	if (!map) return null;
...
function getMetadataValue(mapOrLookup: any, id: any, sessionName: any) {
...
function hasImageFile(files: any) {
	return (files || []).some((file: any) => isImageFile(file.name));
}

function getWasabiSessionFiles(wasabiFiles: any) {
	return wasabiFiles || [];
}

function getDigitalOceanSessionFiles(files: any, wasabiFiles = []) {
...
async function getGroupMetadataFiles(groupName: any) {
...
function getMetadataFingerprint(metadataFiles: any, yearName: any) {
...
async function readYearCache(groupName: any, yearName: any) {
	try {
...
function getCachedSessionsForYear(existingSessions: any, yearName: any) {
...
function getSessionYear(session: any) {
...
function getSessionDate(id: any) {
	const match = String(id || "").match(/^(\d{4}-\d{2}-\d{2})(?:\s|$)/);
	return match?.[1] || null;
}

function mergeSessionsById(existingSessions: any, updatedSessions: any) {
...
export async function updateGroupProcess(
	...
	if (isBundled) {
		...
		if (existingSessions) {
			existingSessions.sort((a: any, b: any) => a.id.localeCompare(b.id));
