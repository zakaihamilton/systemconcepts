...
async function readTextIfExists(path: any) {
...
async function readBinaryIfExists(path: any) {
	const info = await metadataInfo({ path });
	if (!info || info.type === "application/x-directory" || info.type === "dir") {
		return null;
	}
...
export async function aggregateSessionMetadata({ group, year }: any) {
	validateGroup(group);
	validateYear(year);
	...
	const items = (await list({ path: basePath })).map((item: any) =>
