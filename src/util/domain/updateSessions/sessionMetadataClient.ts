...
function getCacheKey(group: any, year: any, metadataFingerprint: any) {
...
async function fetchTextFromUrl(url: any) {
	if (!url) return null;
...
async function fetchBinaryFromUrl(url: any) {
	if (!url) return null;
	const response = await withTimeout(
		fetch(url, { method: "GET" }),
		METADATA_FETCH_TIMEOUT_MS,
...
async function fetchSessionMetadataViaPresign(group: any, year: any) {
...
async function fetchSessionMetadataViaProxy(group: any, year: any) {
	const params = new URLSearchParams({
		group,
		year: String(year),
	});
...
async function loadSessionMetadata(group: any, year: any) {
	try {
		return await withTimeout(
