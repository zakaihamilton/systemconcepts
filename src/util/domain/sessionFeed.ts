...
const transcriptMetadataCache = new Map<any, any>();
...
async function getJsonFile(path: any) {
	const now = Date.now();
...
export async function getSessions({ group }: any = {}) {
...
export function sortSessions(sessions: any) {
...
export function getSProxyUrl(path: any, baseUrl: any) {
	if (!path) return null;
...
function normalizeProxyPath(path: any) {
	return path.replace(/^\//, "").replace(/^aws\//, "");
}

function getMediaPath(session: any) {
...
function getStandaloneTranscriptPath(session: any) {
	const expectedName = `${session.id}.txt`;
	const transcriptName = (session.files || []).find(
		(file: any) => file === expectedName,
	);
...
async function getExistingTranscriptPath(path: any) {
	if (!path) return null;
...
async function getExistingInferredTranscriptPath(session: any) {
...
export async function getTranscriptProxyUrl(session: any, baseUrl: any) {
 ...
 */
export function getTranscriptProxyUrlFast(session: any, baseUrl: any) {
	if (!session) return null;
