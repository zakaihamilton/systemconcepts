...
const sessionRecordCache = new Map<any, any>();
let sessionRecordCacheResetTime = 0;

function parseSessionFileId(fileId: any) {
...
function sessionMatchesId(session: any, { group, year, date, name }: any) {
	return (
		session &&
...
async function readSessionsFromPath(path: any) {
 ...
 */
async function resolveSessionRecord(fileId: any, listSession: any) {
...
async function buildSessionText(session: any) {
 ...
 */
export async function loadParagraphsForFile(fileId: any, sessionsById: any) {
	// Check cache first
	const now = Date.now();
	...
	try {
		...
		const tag = tags.find((t: any) => t._id === fileId);
