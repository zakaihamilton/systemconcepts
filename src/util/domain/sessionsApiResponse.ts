...
export function filterSessions(sessions: any, searchParams: any) {
	...
	if (tag) {
		...
		filtered = filtered.filter((s: any) =>
			(s.tags || []).some((t: any) => t.toLowerCase().trim() === lowerTag),
		);
	}
	if (date) {
		const dateStr = date.trim();
		filtered = filtered.filter((s: any) => s.date === dateStr);
	}
	if (year) {
		const yearStr = year.trim();
		filtered = filtered.filter((s: any) => s.year === yearStr);
	}
	if (query) {
		...
		filtered = filtered.filter(
			(s: any) =>
				...
				(s.tags || []).some((t: any) => t.toLowerCase().includes(lowerQuery)),
		);
	}
...
export async function buildSessionsJson({ sessions, baseUrl }: any) {
