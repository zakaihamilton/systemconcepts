...
let getSProxyUrl: any;
let getSessions: any;
let getTranscriptProxyUrl: any;
let getTranscriptProxyUrlFast: any;
let sortSessions: any;
...
describe("sessionFeed transcript URLs", () => {
	...
	it("reuses transcript metadata lookups for repeated AWS transcript checks", async () => {
		...
		const firstUrl: any = await getTranscriptProxyUrl(
			session,
			"https://systemconcepts.app",
		);
		const secondUrl: any = await getTranscriptProxyUrl(
			session,
			"https://systemconcepts.app",
		);
...
describe("getSessions", () => {
	it("loads manifest and session files, filtering to the requested group", async () => {
		...
		const sessions: any = await getSessions({ group: "american" });
	...
	it("returns sessions from all groups when no group filter is provided", async () => {
		...
		expect(sessions.map((s: any) => s.id).sort()).toEqual(["s1", "s2"]);
	});
	...
	it("includes bundle.json in the file scan and still filters its sessions by group", async () => {
		...
		const sessions: any = await getSessions({ group: "american" });
		expect(sessions.map((s: any) => s.id)).toEqual(["b1"]);
	});
	...
	it("logs and continues when a single session file fails to load", async () => {
		...
		const sessions: any = await getSessions();
		expect(sessions.map((s: any) => s.id)).toEqual(["s1"]);
...
describe("sortSessions", () => {
	it("sorts sessions by date descending", () => {
		...
		expect(sortSessions(sessions).map((s: any) => s.date)).toEqual([
			"2024-03-01",
			"2024-01-01",
		]);
	});
	...
	it("breaks date ties by group name, case-insensitively", () => {
		...
		expect(sortSessions(sessions).map((s: any) => s.group)).toEqual([
			"american",
			"Hebrew",
		]);
	});
	...
	it("breaks group ties by session name", () => {
		...
		expect(sortSessions(sessions).map((s: any) => s.name)).toEqual([
			"Alpha",
			"Zeta",
		]);
	});
...
describe("getTranscriptProxyUrlFast", () => {
	...
	it("prefers an explicit subtitles path", () => {
		const url: any = getTranscriptProxyUrlFast(
