...
export function splitSmart(txt: any) {
	const chunks = [];
	let remaining = txt;
	while (remaining) {
...
export function mergeChunks(chunks: any) {
	if (chunks.length === 0) return chunks;
	const merged = [chunks[0]];
	const getType = (text: any) => {
...
export function splitIntoParagraphs(text: any) {
	return mergeChunks(splitSmart(text));
}
