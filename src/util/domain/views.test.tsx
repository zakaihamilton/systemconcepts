...
const FIXTURE_PAGES: any[] = [
	...
	{ id: "editor", name: "EDITOR" },
	{
		id: "user",
		name: "USER",
		section: ({ name }: any) => (name ? { name: `User: ${name}` } : null),
	},
	{
		id: "player",
		name: "PLAYER",
...
describe("getPagesFromHash", () => {
	...
	it("resolves multiple segments and injects the root page when missing", () => {
		...
		expect(results.map((page: any) => page.id)).toEqual(["home", "settings"]);
	...
	it("resolves an allowed contained sub-page", () => {
		...
		expect(results.map((page: any) => page.id)).toEqual([
			"home",
			"storage",
			"editor",
		]);
	...
	it("returns null (skips the segment) when no page matches", () => {
		...
		expect(results.map((page: any) => page.id)).toEqual(["home"]);
	});
	...
	it("resolves query parameters into the page object", () => {
		...
		const userPage = results.find((page: any) => page.id === "user");
	...
	it("matches custom pages using their id as a pattern", () => {
		...
		expect(results.map((page: any) => page.id)).toEqual(["home", "research"]);
	});
	...
	it("uses sectionPath prefixes and useParentName when allowed contained pages have a name param", () => {
		const pages = [
			...
			},
			{
				id: "editor",
				name: "EDITOR",
				section: ({ name }: any) => ({ name: name || "EDITOR" }),
			},
		];
		const results = getPagesFromHash({
		...
		const editor = results.find((page: any) => page.id === "editor");
	...
	it("falls back to pageId matching when custom pattern misses", () => {
		...
		expect(results.map((p: any) => p.id)).toEqual(["home", "player"]);
	});
	...
	it("keeps an existing tooltip when the resolved name changes", () => {
		const pages = [
			{ id: "home", root: true, name: "HOME" },
			{
				...
				name: "USER",
				tooltip: "Existing",
				section: ({ name }: any) => ({ name: `User: ${name}` }),
			},
		];
		const results = getPagesFromHash({
		...
		expect(results.find((p: any) => p.id === "user").tooltip).toBe("Existing");
	});
	...
	it("extends sectionPath across three allowed nested segments", () => {
		...
		const preview = results.find((page: any) => page.id === "preview");
	...
	it("works without a root page definition", () => {
		...
		expect(results.map((p: any) => p.id)).toEqual(["settings"]);
	});
});
...
describe("usePages / useActivePages / useCurrentPage / useCurrentPageTitle", () => {
	it("usePages maps translation keys and filters hidden pages", async () => {
		...
		await waitFor(() => {
			...
			const settingsPage = pages.find((page: any) => page.id === "settings");
	...
	it("usePages maps localized object names and mode-specific icons", async () => {
		...
		await waitFor(() => {
			const pages = getResult();
			expect(pages.find((p: any) => p.id === "hidden")).toBeUndefined();
			expect(pages.find((p: any) => p.id === "modeful").name).toBe("Side Name");
			expect(pages.find((p: any) => p.id === "undef-visible")).toBeTruthy();
		});
	...
	it("usePages maps Hebrew object names when language is heb", async () => {
		...
		const getResult = renderHook(() => usePages());
		await waitFor(() =>
			expect(getResult().find((p: any) => p.id === "hebrew").name).toBe(
				"Hebrew",
			),
		);
	...
	it("useActivePages accepts a depends array", async () => {
		...
		await waitFor(() =>
			expect(getResult().map((page: any) => page.id)).toEqual([
				"home",
				"settings",
			]),
		);
