...
describe("toggleProperty", () => {
	it("uses the first configured value when the current token is not in the list", () => {
		expect(toggleProperty("--test-token", ["a", "b"])).toBe("a");
		expect(getProperty("--test-token").trim()).toBe("a");
	});
});
...
describe("useStyles", () => {
	...
	it("evaluates function values using the full data object", () => {
		const { result } = renderHook(() =>
			useStyles(styles, {
				active: (data: any) => data.highlighted,
				highlighted: true,
			}),
		);
