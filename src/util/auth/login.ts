...
export async function login({ id, password, api, path }: any) {
	if (!id) {
...
export async function resetPassword({ id, code, newPassword, salt = 10 }: any) {
	id = id.toLowerCase();
	let user = null;
	try {
		user = await findRecord({ collectionName: "users", query: { id } });
...
export async function sendResetEmail({ id }: any) {
	id = id.toLowerCase();
	let user = null;
	try {
		user = await findRecord({ collectionName: "users", query: { id } });
