...
function requireCurrentChallenge(challengeRecord: any) {
...
export async function verifyPasskeyRegistration({
	...
	if (verification.verified) {
		...
		const newCredential = {
			...
			name:
				name ||
				`Passkey ${user && user.credentials ? user.credentials.length + 1 : 1}`,
...
export async function getPasskeys({ id }: any) {
	if (!id) {
		throw "MISSING_ID";
	}
	id = id.toLowerCase();
	...
	return (user.credentials || []).map((cred: any) => ({
		id: cred.id,
...
export async function deletePasskey({ id, credentialId }: any) {
	if (!id) {
		throw "MISSING_ID";
	}
	id = id.toLowerCase();
...
export async function getPasskeyAuthOptions({ id, rpID }: any) {
	let user = null;
	if (id) {
		id = id.toLowerCase();
...
export async function verifyPasskeyAuth({ id, response, origin, rpID }: any) {
	if (!id) {
		throw "MISSING_ID";
	}
	id = id.toLowerCase();
	...
	const credential = (user.credentials || []).find((c: any) => c.id === credId);
