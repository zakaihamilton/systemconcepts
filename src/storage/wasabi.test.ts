...
describe("getListing", () => {
	it("maps device-prefixed ids and paths onto each entry", async () => {
		...
		expect(asMock(fetchJSON).mock.calls[0][1]).toMatchObject({
			method: "GET",
			cache: "no-store",
		});
	});
...
describe("readFile", () => {
	it("fetches text content for non-binary files using the stable cache options", async () => {
		asMock(fetchText).mockResolvedValue("hello world");
		...
		const [url, options] = asMock(fetchText).mock.calls[0];
	...
	it("fetches and decodes binary content", async () => {
		...
		asMock(fetchBlob).mockResolvedValue(blob);
		asMock(binaryToString).mockReturnValue("encoded");
