...
const readCache = new Map<any, any>();

async function cachedRead(key: any, loader: any) {
	const now = Date.now();
...
function invalidateReadCache(path: any) {
...
async function writeFile(path: any, body: any) {
	path = makePath(path)
		.replace(/^\/aws\//, "/")
		.replace(/^aws\//, "");
	...
	try {
		...
		if (
			...
			const view = body as ArrayBufferView<ArrayBufferLike>;
			const bytes = new Uint8Array(view.byteLength);
			bytes.set(new Uint8Array(view.buffer, view.byteOffset, view.byteLength));
			bodyToSend = await binaryToString(new Blob([bytes]));
		}
...
async function getRecursiveList(path: any, options: Record<string, any> = {}) {
	path = makePath(path);
